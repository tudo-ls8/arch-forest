...
class OptimizedIFTreeConverter(TreeConverter):
    ...
    def sizeOfNode(self, tree, node, splitDataType):
       if(self.nodeSizeTable[node.id] != -1):
           return self.nodeSizeTable[node.id]
    ...
    def getCode(self, tree, treeID, numClasses):
        ...
        #print the all node size sum in the node size table
        allSize = 0
        for i in range (len(self.nodeSizeTable)):
            allSize += self.nodeSizeTable[i]
        anFile = open("allNode", "a")
        anFile.write(str(treeID) + ' ' + str(allSize) + '\n')
        anFile.close()
    ...
    def createCppSectionFile(self, tree):
        ...
        for i in range(len(tree.nodes)):
            self.nodeSizeTable.append(-1)
        #empty section define
        cppStr = self.getEmptySection()
        #create section file by nodes
        for i in range(len(tree.nodes)):
        ...
        f.write(cppStr)
        f.close()
        os.system("g++ -c -O3 sizeOfNode.cpp")
